...
function OperatorLogin({ onConfirm }: { onConfirm: (name: string) => void }) {
  ...
  const handleSubmit = () => {
    ...
    onConfirm(name); // ← operatorName 상태를 Provider에서 업데이트
  };
...
export function OperatorProvider({ children }: { children: React.ReactNode }) {
  // 규칙 1: 무조건 null로 시작
  // 규칙 2: useEffect 없음 — localStorage 자동 읽기 완전 금지
  const [operatorName, setOperatorName] = useState<string | null>(null);

  // 규칙 4: onConfirm이 호출되면 operatorName을 정확히 업데이트
  ...
  // 규칙 3-A: operatorName === null → OperatorLogin만 렌더
  if (operatorName === null) {
    return <OperatorLogin onConfirm={handleConfirm} />;
  }

  // 규칙 3-B: operatorName에 값이 있을 때만 children 렌더
  return (
