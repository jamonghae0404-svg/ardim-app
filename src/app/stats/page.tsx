...
import {
  ...
  BookOpen, Activity, Layers, Download, SlidersHorizontal, CalendarRange,
} from "lucide-react";
...
import { type Program, type Member, type AttendanceData } from "@/lib/storage";
...
export default function StatsPage() {
  ...
  const monthDates = useMemo(
    () => isAnnual ? [] : getMonthDates(selectedYear, selectedMonth),
    [selectedYear, selectedMonth, isAnnual],
  );
  const weekDates = useMemo(
    () => isAnnual ? [] : getSelectedWeekDates(selectedYear, selectedMonth, selectedWeek),
    [selectedYear, selectedMonth, selectedWeek, isAnnual],
  );
  const yearDates = useMemo(
    () => isAnnual ? getYearDates(selectedYear) : [],
    [selectedYear, isAnnual],
  );
  ...
  const yearStats = useMemo(
    () => calcPeriodStats(yearDates, targetMembers, attendanceData),
    [yearDates, targetMembers, attendanceData],
  );
  ...
  const annualBarData = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_, i) => {
      ...
      const count = dates.reduce((sum, date) => {
        return sum + Object.entries(attendanceData[date] ?? {})
          .filter(([id, s]) => s === "present" && idSet.has(id)).length;
      }, 0);
      return { month, count };
    });
  }, [isAnnual, selectedYear, targetMembers, attendanceData]);
  ...
  }, [weekDates, targetMembers, attendanceData, isAnnual]);
  const maxBar = Math.max(...barData.map((d) => d.count), ...annualBarData.map((d) => d.count), 1);

  const programBreakdown = useMemo(() => {
    const dates = isAnnual ? yearDates : monthDates;
    return programs.map((p) => ({
      program: p,
      ...calcPeriodStats(dates, members.filter((m) => m.programId === p.id), attendanceData),
    }));
  }, [programs, members, monthDates, yearDates, attendanceData, isAnnual]);
  ...
  const isCurrentMonth = !isAnnual && selectedYear === CURRENT_YEAR && selectedMonth === CURRENT_MONTH;
      ...
      <Card className="border-0 bg-white shadow-sm">
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <SelectTrigger className={`h-9 rounded-xl border-gray-200 text-xs ${isAnnual ? "w-44 border-blue-300 bg-blue-50 text-blue-700" : "w-20"}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {MONTH_OPTIONS.map((m) => (
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
            ...
            <span className="text-gray-300">·</span>
            <span className={`px-2 py-0.5 rounded-full font-medium ${isAnnual ? "bg-indigo-50 text-indigo-600" : "bg-violet-50 text-violet-600"}`}>
              {monthLabel}
              {isCurrentMonth && (
      ...
      {isLoading ? <Spinner /> : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          {isAnnual ? (
            <>
              {/* 연간 합산 카드 */}
              <Card className="col-span-2 border-0 bg-gradient-to-br from-indigo-600 to-indigo-700 text-white shadow-lg shadow-indigo-200/50">
                <CardContent className="p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <p className="text-[11px] font-semibold text-indigo-200 uppercase tracking-wide">연간 통계</p>
                      <p className="text-[10px] text-indigo-300 mt-0.5">{selectedYear}년 1월 ~ 12월</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/10 rounded-xl p-3">
                      <p className="text-indigo-200 text-[11px] font-medium">실인원</p>
                      <p className="text-4xl font-bold mt-0.5">{yearStats.실인원}</p>
                      <p className="text-indigo-300 text-[10px] mt-1">중복 제거 인원</p>
                    </div>
                    <div className="bg-white/10 rounded-xl p-3">
                      <p className="text-indigo-200 text-[11px] font-medium">연인원</p>
                      <p className="text-4xl font-bold mt-0.5">{yearStats.연인원}</p>
                      <p className="text-indigo-300 text-[10px] mt-1">누적 출석 인원</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 bg-indigo-50 shadow-sm">
                <CardContent className="p-4">
                  <div className="flex items-center gap-1.5 mb-3">
                    ...
                    <span className="text-[11px] text-indigo-600 font-semibold">연간 진행</span>
                  </div>
                  <p className="text-4xl font-bold text-indigo-700">{yearStats.진행횟수}</p>
                  <p className="text-xs text-indigo-400 mt-1">회</p>
                </CardContent>
              </Card>
              ...
              <Card className="border-0 bg-amber-50 shadow-sm">
                <CardContent className="p-4">
                  <div className="flex items-center gap-1.5 mb-3">
                    ...
                    <span className="text-[11px] text-amber-600 font-semibold">연간 출석률</span>
                  </div>
                  <p className="text-4xl font-bold text-amber-700">{calcRate(yearStats)}</p>
                          ...
                          <div className="w-full flex items-end justify-center" style={{ height: 64 }}>
                            <div
                              className={`w-full rounded-t-md transition-all duration-500 ${
                                count === 0 ? "bg-gray-100" : "bg-indigo-400"
                              }`}
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="text-[11px] font-semibold text-blue-200 uppercase tracking-wide">주차 통계</p>
              ...
              <Card className="col-span-2 border-0 bg-gradient-to-br from-violet-600 to-violet-700 text-white shadow-lg shadow-violet-200/50">
                <CardContent className="p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <p className="text-[11px] font-semibold text-violet-200 uppercase tracking-wide">월간 통계</p>
            ...
            <CardHeader className="pb-2 px-5 pt-5">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  ...
                  프로그램별 현황
                  <span className="text-xs font-normal text-gray-400">({monthLabel})</span>
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent className="px-5 pb-5">
